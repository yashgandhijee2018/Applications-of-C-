 ...
 class point
 {
     ...
     ld manhattan(ld a,ld b)
     {
         ld res=abs(x-a)+abs(y-b);
         return res;
     }
     ld manahttan(point p)
     {
         ld a=p.x;
         ld b=p.y;
         ld res=abs(x-a)+abs(y-b);
 ...
 int main()
 {
     ...
     rep(itr,10)
     {
         ...
         rep(i,n)
         {
             ...
             {
                 dis[curr]=a[i].manahttan(centroids[curr]);
                 if(dis[curr]<disMin)
                 {
                     disMin=dis[curr];
                     pos=curr;
             ...
             cout<<"\nThe manhattan dist of point ";a[i].getPoint();
         ...
         cout<<"\nCentroid means after this iteration:\n";
         rep(i,k)
         {
             ll s=result[i].size();
             point temp;
             ...
             rep(j,s)
             {
                 temp.x+=a[result[i][j]-1].x;
                 temp.y+=a[result[i][j]-1].y;
             }
             temp.x/=s;
             temp.y/=s;
             centroids[i]=temp;
